...
//            objectValue.push_back(row[j]);
            objectValue.emplace_back(row[j]);
        }
...
unsigned int MysqlManager::update(std::string sql) {
    if(!IsConnected) {
        cout << "" << endl;
        return -1;
    }
    ...
    if(res >= 0){
        // 返回受影响的行数
        rows = mysql_affected_rows(&mySQLClient);
        cout << "Update "<< rows << " rows\n";
        return rows;
    } else {
        cout << "Update error " << mysql_errno(&mySQLClient) << "," << mysql_error(&mySQLClient) << endl;
        return -1;
    }
}

unsigned int MysqlManager::deleteRecord(std::string sql) {
    if(!IsConnected) {
        cout << "" << endl;
        return -1;
    }
    ...
    if(res >= 0){
        // 返回受影响的行数
        rows = mysql_affected_rows(&mySQLClient);
        cout << "Delete "<< rows << " rows\n";
        return rows;
    } else {
        cout << "Delete error " << mysql_errno(&mySQLClient) << "," << mysql_error(&mySQLClient) << endl;
        return -1;
    }
}
...
void MysqlManager::printResult(std::vector<std::vector<std::string> > result) {
    for ( auto &res : result ) {
        for ( auto &r : res ) {
            cout << r << "\t";
        }
